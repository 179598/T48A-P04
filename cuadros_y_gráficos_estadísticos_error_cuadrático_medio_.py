# -*- coding: utf-8 -*-
"""Cuadros y gráficos estadísticos   Error cuadrático medio .ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1CgFwlQPWRpV49sTeaHapy3_EkUV8QHKY
...
# Datos de los porcentajes de juegos ganados (Pct) extraídos de la imagen
porcentajes_ganados = np.array([
    0.688, 0.688, 0.563, 0.250, 0.125,  # AFC East
    0.688, 0.688, 0.438, 0.313,        # AFC Central
    0.688, 0.625, 0.563, 0.438, 0.125, # AFC West
    0.813, 0.688, 0.563, 0.375, 0.250, # NFC East
    0.688, 0.563, 0.500, 0.313, 0.188, # NFC Central
    0.875, 0.375, 0.313, 0.188         # NFC West
])

# Definir los límites de las clases (cinco clases de igual tamaño)
min_pct, max_pct = min(porcentajes_ganados), max(porcentajes_ganados)
clases = np.linspace(min_pct, max_pct, num=6)  # 5 clases => 6 puntos de corte

# Obtener la frecuencia absoluta en cada clase
frecuencia_absoluta, _ = np.histogram(porcentajes_ganados, bins=clases)
...
for i in range(len(frecuencia_absoluta)):
    print(f"{clases[i]:.3f} - {clases[i+1]:.3f}\t{frecuencia_absoluta[i]}\t\t{frecuencia_relativa[i]:.6f}")
...
plt.plot(puntos_medios, frecuencia_absoluta, marker='o', linestyle='-', color='royalblue', label="Frecuencia Absoluta")
...
# Calcular la frecuencia acumulada con el término "mayor que"
frecuencia_acumulada_mayor_que = np.cumsum(frecuencia_absoluta[::-1])[::-1]

# Construcción de la ojiva "mayor que"
plt.figure(figsize=(10, 5))
plt.step(clases[:-1], frecuencia_acumulada_mayor_que, where='post', marker='o', linestyle='-', color='red', label="Frecuencia Acumulada (Mayor que)")
